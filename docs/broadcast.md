...
**Persist + broadcast state-handoff race.** When `persist` and `broadcast` are composed on the same store and the new broadcaster tab also has a pending persist hydration, there is a known ordering issue: the new broadcaster applies the `state-handoff` payload from the yielding tab (updating signals to the current in-memory state), but if persist hydration resolves afterwards, it will overwrite that state with the older stored values. To avoid stale state after a broadcaster change, use `onHydrated` to trigger a re-broadcast after hydration, or prefer `many-to-many` mode where every tab always has the latest value.
